		...
		Connections["Slide"] = RunService.RenderStepped:Connect(function(deltaTime)
			...
			if Instance then
				if
					...
					and Instance.CanCollide == true
					and (Instance.Parent.Name ~= "Vault" and Instance.Parent.Parent.Name ~= "Vault")
					and Instance.Name ~= "SlideBarrier"
				then
...
function module:BeginSlide()
	if not Character:GetAttribute("Sliding") then
		...
		VerifySlide:andThen(function(CanSlide, SlideLifeTime)
			if CanSlide and Humanoid.MoveDirection ~= Vector3.new() then
