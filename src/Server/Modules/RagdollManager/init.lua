...
RagdollModule.BuildRagdoll = require(script.BuildRagdoll)
...
function setRagdollEnabled(humanoid, isEnabled)
	pcall(function()
		...
		for _,constraint in ragdollConstraints:GetChildren() do
			if constraint:IsA("Constraint") and constraint.Attachment1 then
				constraint.Enabled = isEnabled --// this is mostly for the RigidConstraint, so that it doesnt have to be Enabled all the time and make humanoid movement stiff
				local rigidJoint = constraint.RigidJoint.Value --// the reason there is a pcall...
...
for _,humanoid in collectionService:GetTagged(TAG_NAME) do
